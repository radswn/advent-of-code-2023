...
row_offsets = [sum(row in empty_rows for row in range(i)) * (expansion_rate - 1) for i in range(map_size[0])]
column_offsets = [sum(col in empty_columns for col in range(j)) * (expansion_rate - 1) for j in range(map_size[1])]
